...
/* Разделы готовности: какие тесты в какой складываются.
   ...
   Раздела «формат» тут нет и быть не должно: в оценку идёт только то,
   что на экзамене действительно оценивают. Справочное знание про
   структуру экзамена — условие входа, а не измеряемый навык. */
const SECTIONS = [
...
function estimate(stats) {
    ...
    return {
      ...
      empty: {
        ...
        note: `Пройдено тестов: ${takenTests} из ${stats.tests.length}, отвечено вопросов: ${answered}. ` +
              'Начни с «Рубеж · Чтение» — приёмы поиска дают прибавку быстрее всего, ' +
              'и сразу видно, какие из них уже работают.',
        href: '#/test/part2',
        cta: 'Начать с чтения',
      },
    };
  ...
  /* Общий ориентир — среднее по разделам с весами, а не по всем вопросам
     подряд: иначе двенадцать вопросов по лексике перевесили бы письмо,
     которого вдвое меньше в пуле и вдвое больше на экзамене. */
